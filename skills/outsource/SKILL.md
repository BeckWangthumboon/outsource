...
3. Run `outsource launch --prompt "..." --json`. Add each visual reference with a repeated `--image <path-or-https-url>` flag.
...
Use `outsource status <agent-id> --json` for one debugging snapshot. Do not poll continuously.

Use `outsource follow-up <agent-id> --prompt "..." --json` only for a concrete correction or continuation. Repeat `--image` for visual context. If the CLI reports `agent_busy`, follow the active work rather than retrying blindly.
